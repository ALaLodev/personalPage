...
const translations = {
    ...
    javaDesc2: { es: 'Spring WebFlux, Kafka, Prometheus/Grafana. TDD con JUnit 5 y Mockito.', en: 'Spring WebFlux, Kafka, Prometheus/Grafana. TDD with JUnit 5 and Mockito.' },
    projectsLabel: { es: 'Proyectos', en: 'Projects' },
    projectsTitle: { es: 'Trabajo reciente.', en: 'Recent work.' },
    skylinkMobileFeatured: { es: 'Destacado', en: 'Featured' },
    skylinkMobileTitle: { es: 'SkyLink App — Pasajeros Android', en: 'SkyLink App — Android Passengers' },
    skylinkMobileDesc: { es: 'Aplicación móvil nativa para pasajeros de SkyLink GDS desarrollada en Kotlin con interfaz 100% declarativa en Jetpack Compose bajo Clean Architecture y patrón MVVM estricto. Implementa gestión asíncrona reactiva con Coroutines y StateFlow, inyección de dependencias con Dagger Hilt y almacenamiento seguro de credenciales JWT a nivel de hardware con Jetpack Security (EncryptedSharedPreferences). Permite búsqueda dinámica de vuelos, selección de asientos por cabina y gestión de tarjetas de embarque digitales conectada al API Gateway.', en: 'Native passenger mobile application for the SkyLink GDS platform. Built with Kotlin featuring a 100% declarative UI in Jetpack Compose under Clean Architecture and strict MVVM pattern. Implements reactive asynchronous state with Coroutines and StateFlow, dependency injection via Dagger Hilt, and hardware-backed JWT credential storage using Jetpack Security (EncryptedSharedPreferences). Enables dynamic flight discovery, cabin seat selection, and digital boarding pass management connected to the API Gateway.' },
    skylinkMobileVer: { es: 'Ver app móvil', en: 'View mobile app' },
    skylinkMobileBadge: { es: 'Android Nativo', en: 'Native Android' },
    skylinkAdminFeatured: { es: 'Destacado', en: 'Featured' },
    skylinkAdminTitle: { es: 'SkyLink GDS — Backend & Dashboard B2B', en: 'SkyLink GDS — Backend & B2B Dashboard' },
    skylinkAdminDesc: { es: 'Ecosistema distribuido y panel de control para aerolíneas. Backend en Java 21 con Spring Boot y Spring Cloud (Eureka, Gateway reactivo con Circuit Breakers Resilience4j y Rate Limiting en Redis), orquestación asíncrona mediante Saga Choreography con Apache Kafka y observabilidad integral con Prometheus, Grafana y Zipkin. Frontend corporativo en Angular 21 (arquitectura standalone, Zoneless, mapa interactivo de asientos en Boeing 787-9 y monitorización de salud del clúster con streaming de telemetría y logs).', en: 'Distributed ecosystem and operations dashboard for airlines. Backend in Java 21 with Spring Boot and Spring Cloud (Eureka, reactive Gateway with Resilience4j Circuit Breakers and Redis Rate Limiting), asynchronous orchestration via Saga Choreography with Apache Kafka, and full observability with Prometheus, Grafana, and Zipkin. Enterprise frontend in Angular 21 (standalone architecture, Zoneless, interactive Boeing 787-9 seat map, and cluster health monitoring with real-time log streaming).' },
    skylinkAdminVer: { es: 'Ver dashboard', en: 'View dashboard' },
    ...
    exp1Date: { es: 'Febrero 2026 — Actualidad', en: 'February 2026 — Present' },
