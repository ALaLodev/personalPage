...
const translations = {
    ...
    skylinkFeatured: { es: 'Destacado', en: 'Featured' },
    skylinkTitle: { es: 'SkyLink GDS — Global Distribution System', en: 'SkyLink GDS — Global Distribution System' },
    skylinkDesc: { es: 'Ecosistema empresarial distribuido completo para la industria aérea (GDS). Arquitectura de microservicios en Java 21 con Spring Boot, Spring Cloud (Eureka, Gateway reactivo con Circuit Breakers y Rate Limiting en Redis), orquestación asíncrona mediante Saga Choreography con Apache Kafka y observabilidad integral con Prometheus, Grafana y Zipkin. Incluye dashboard corporativo B2B en Angular 21 (arquitectura standalone, mapa de asientos interactivo en tiempo real y monitorización de salud de clúster) y aplicación móvil nativa para pasajeros en Kotlin con Jetpack Compose y Clean Architecture.', en: 'Comprehensive distributed enterprise ecosystem for the airline industry (GDS). Microservices architecture in Java 21 with Spring Boot, Spring Cloud (Eureka, reactive Gateway with Circuit Breakers and Redis Rate Limiting), asynchronous orchestration via Saga Choreography with Apache Kafka, and full observability with Prometheus, Grafana, and Zipkin. Features an enterprise B2B dashboard in Angular 21 (standalone architecture, interactive real-time seat map, and cluster health monitoring) and a native passenger mobile app in Kotlin with Jetpack Compose and Clean Architecture.' },
    skylinkVer: { es: 'Ver proyecto', en: 'View project' },
